...
|| (isset($_POST["sub_sticker_2"])) && !empty($_POST["sub_sticker_2"]) && ($_POST["sub_sticker_2"] !== ' ') && substr($_POST["sub_sticker_2"], 0, 5) !== "http:")) {
    ...
    echo '<script type=text/javascript>alert("Image successfully updated"); window.location="http://localhost/Camagru/gallery/gall.php";</script>';
}
else
    echo '<script type=text/javascript>alert("You must either take a photo or upload an image AND choose either a sticker or a background"); window.location="http://localhost/Camagru/main_page/mp.php";</script>';
?>